...
ENTRUSTED_ARCH=$(cat $HOME/LIVE_BOOT-${DEBIAN_ARCH}/chroot/etc/entrusted_arch | head -1)
...
cp "${HOME}/LIVE_BOOT-${DEBIAN_ARCH}/entrusted-livecd-${ENTRUSTED_ARCH}-${ENTRUSTED_VERSION}.iso" "${PROJECTDIR}/artifacts/entrusted-livecd-${CPU_ARCH}-${ENTRUSTED_VERSION}.iso"
