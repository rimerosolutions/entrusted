...
podman run --rm -v "${ARTIFACTSDIR}":/build docker.io/binfalse/nsis installer.nsi || (sleep 10 && podman run --rm -v "${ARTIFACTSDIR}":/build docker.io/binfalse/nsis installer.nsi)
retVal=$?
if [ $retVal -ne 0 ]; then
