...
for CPU_ARCH in ${CPU_ARCHS} ; do
    for LIBC_FLAVOR in ${LIBC_FLAVORS} ; do
        ARTIFACTSDIR="${PROJECTDIR}/../artifacts/entrusted-${APPVERSION}-linux-${CPU_ARCH}-${LIBC_FLAVOR}"
...
for CPU_ARCH in $CPU_ARCHS ; do    
    ...
    # MUSL    
    cp "${PROJECTDIR}/../artifacts/entrusted-${APPVERSION}-linux-${CPU_ARCH}-glibc"/* "${PROJECTDIR}/../artifacts/entrusted-${APPVERSION}-linux-${CPU_ARCH}-musl/"
    rm "${PROJECTDIR}/../artifacts/entrusted-${APPVERSION}-linux-${CPU_ARCH}-musl/entrusted-gui"
    
    podman run --rm --platform linux/${DEB_ARCH} -v "${PROJECTDIR}":/src  -v "${PROJECTDIR}/../artifacts":/artifacts -v "${PROJECTDIR}/../ci_cd":/ci_cd docker.io/uycyjnzgntrn/alpine:${ALPINE_VERSION} /bin/sh -c "apk add rust cargo git cmake make g++ pango-dev fontconfig-dev libxinerama-dev libxfixes-dev libxcursor-dev libpng-dev cairo-dev librsvg-dev wayland-dev wayland-protocols wayland-libs-client wayland-libs-cursor wayland-libs-egl libxkbcommon-dev zlib-dev wayland-libs-egl mesa-egl mesa-dev dbus-dev curl && cargo clean --manifest-path /src/entrusted_client/Cargo.toml && CFLTK_BUNDLE_URL='${CFLTK_BUNDLE_URL}' cargo build --release --features=gui,fltk/use-wayland,fltk/fltk-bundled --manifest-path /src/entrusted_client/Cargo.toml --bin entrusted-gui && cp /src/entrusted_client/target/release/entrusted-gui /artifacts/entrusted-${APPVERSION}-linux-${CPU_ARCH}-musl/ && strip /artifacts/entrusted-${APPVERSION}-linux-${CPU_ARCH}-musl/entrusted-gui && cargo clean --manifest-path /src/entrusted_client/Cargo.toml" || (sleep 10 && podman run --rm --platform linux/${DEB_ARCH} -v "${PROJECTDIR}":/src  -v "${PROJECTDIR}/../artifacts":/artifacts -v "${PROJECTDIR}/../ci_cd":/ci_cd docker.io/uycyjnzgntrn/alpine:${ALPINE_VERSION} /bin/sh -c "apk add rust cargo git cmake make g++ pango-dev fontconfig-dev libxinerama-dev libxfixes-dev libxcursor-dev libpng-dev cairo-dev librsvg-dev wayland-dev wayland-protocols wayland-libs-client wayland-libs-cursor wayland-libs-egl libxkbcommon-dev zlib-dev wayland-libs-egl mesa-egl mesa-dev dbus-dev curl && cargo clean --manifest-path /src/entrusted_client/Cargo.toml && CFLTK_BUNDLE_URL='${CFLTK_BUNDLE_URL}' cargo build --release --features=gui,fltk/use-wayland,fltk/fltk-bundled --manifest-path /src/entrusted_client/Cargo.toml --bin entrusted-gui && cp /src/entrusted_client/target/release/entrusted-gui /artifacts/entrusted-${APPVERSION}-linux-${CPU_ARCH}-musl/ && strip /artifacts/entrusted-${APPVERSION}-linux-${CPU_ARCH}-musl/entrusted-gui && cargo clean --manifest-path /src/entrusted_client/Cargo.toml")
    ...
    for LIBC_FLAVOR in ${LIBC_FLAVORS_PKGS} ; do
        podman run --rm --platform linux/${DEB_ARCH} -v "${PROJECTDIR}":/src  -v "${PROJECTDIR}/../artifacts":/artifacts  -v "${PROJECTDIR}/../ci_cd":/ci_cd docker.io/uycyjnzgntrn/rust-linux:${RUST_CI_VERSION} /bin/sh -c "cd /src && apt update && /ci_cd/linux/redhat.sh ${APPVERSION} /artifacts/entrusted-${APPVERSION}-linux-${CPU_ARCH}-${LIBC_FLAVOR}.rpm /src/images /artifacts/entrusted-${APPVERSION}-linux-${CPU_ARCH}-${LIBC_FLAVOR} ${RPM_ARCH} && /ci_cd/linux/debian.sh ${APPVERSION} /artifacts/entrusted-${APPVERSION}-linux-${CPU_ARCH}-${LIBC_FLAVOR}.deb /src/images /artifacts/entrusted-${APPVERSION}-linux-${CPU_ARCH}-${LIBC_FLAVOR} ${DEB_ARCH}" || (sleep 10 && podman run --rm --platform linux/${DEB_ARCH} -v "${PROJECTDIR}":/src  -v "${PROJECTDIR}/../artifacts":/artifacts  -v "${PROJECTDIR}/../ci_cd":/ci_cd docker.io/uycyjnzgntrn/rust-linux:${RUST_CI_VERSION} /bin/sh -c "cd /src && apt update && /ci_cd/linux/redhat.sh ${APPVERSION} /artifacts/entrusted-${APPVERSION}-linux-${CPU_ARCH}-${LIBC_FLAVOR}.rpm /src/images /artifacts/entrusted-${APPVERSION}-linux-${CPU_ARCH}-${LIBC_FLAVOR} ${RPM_ARCH} && /ci_cd/linux/debian.sh ${APPVERSION} /artifacts/entrusted-${APPVERSION}-linux-${CPU_ARCH}-${LIBC_FLAVOR}.deb /src/images /artifacts/entrusted-${APPVERSION}-linux-${CPU_ARCH}-${LIBC_FLAVOR} ${DEB_ARCH}")
        ...
        cd ${PROJECTDIR}/../artifacts && tar cvf entrusted-${APPVERSION}-linux-${CPU_ARCH}-${LIBC_FLAVOR}.tar entrusted-${APPVERSION}-linux-${CPU_ARCH}-${LIBC_FLAVOR}
    done
done
