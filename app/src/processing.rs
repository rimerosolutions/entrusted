...
use crate::l10n;
use crate::common::{EventSender, AppEvent};
...
pub fn progressed(tx: &Box<dyn EventSender>, value: usize, message: String) {
    let _ = tx.send(AppEvent::ConversionProgressEvent(value, message));
}

pub fn execute(ctx: ExecCtx) -> Result<(), Box<dyn Error>> {
    ...
    let input_file_path = input_as_pdf_to_pathbuf_uri(root_tmp_dir.clone(), target_dpi, &tx, &progress_range, raw_input_path, document_password.clone(), l10n.clone())?;
    ...
    let (template_img_to_pdf, ocr_settings) = if let Some(v) = ctx.ocr_lang {
        ...
        ("Performing OCR on page {0}", Some(tess_settings))
    } else {
    ...
    for i in 0..page_count {
        ...
        progressed(&tx, progress_value, l10n.gettext_fmt("Extracting page {0} into a PNG image", vec![&page_num_text]));
        let doc_img = page_to_pixmap(i, &doc, target_dpi)?;

        progressed(&tx, progress_value, l10n.gettext_fmt(template_img_to_pdf, vec![&page_num_text]));
        doc_to_pdf(i, ocr_settings.clone(), &doc_img, output_dir_path.clone())?;
    }

    // step 4 (90%-98%)
    progress_range.update(90, 98);
    merge_pdfs(&tx, &progress_range, page_count, output_dir_path, output_file_path.clone(), l10n.clone())?;

    // step 5 (98%-98%)
    progress_range.update(98, 98);
    move_file_to_dir(&tx, &progress_range, output_file_path, safe_dir_path, l10n.clone())?;

    let millis = timer.elapsed().as_millis();
    progressed(&tx, 100, format!("{}: {}", l10n.gettext("Elapsed time"), elapsed_time_string(millis, l10n.clone())));
...
fn move_file_to_dir(tx: &Box<dyn EventSender>, progress_range: &ProgressRange, src_file_path: PathBuf, dest_dir_path: PathBuf, l10n: l10n::Translations) -> Result<(), Box<dyn Error>> {
    if let Err(ex) = fs::copy(&src_file_path, &dest_dir_path) {
        progressed(tx, progress_range.min, l10n.gettext_fmt("Failed to copy file from {0} to {1}", vec![&src_file_path.display().to_string(), &dest_dir_path.display().to_string()]));
        return Err(ex.into());
    }

    if let Err(ex) = fs::remove_file(&src_file_path) {
        progressed(tx, progress_range.min, l10n.gettext_fmt("Failed to remove file from {0}.", vec![&src_file_path.display().to_string()]));
        return Err(ex.into());
    }

    progressed(tx, progress_range.min, l10n.gettext("Moving output files to their final destination"));

    Ok(())
}

fn input_as_pdf_to_pathbuf_uri(root_tmp_dir: PathBuf, _: f32, tx: &Box<dyn EventSender>, _: &ProgressRange, raw_input_path: PathBuf, opt_passwd: Option<String>, l10n: l10n::Translations) -> Result<String, Box<dyn Error>> {
    ...
    if let Some(mime_type) = file_format.short_name() {
        if let Some(parent_dir) = raw_input_path.parent() {
            ...
            match mime_type {
                "PDF" => {
                    progressed(tx, 5, l10n.gettext_fmt("Copying PDF input to {0}", vec![&filename_pdf]));
                ...
                "BMP" | "PNM" | "PNG" | "JPEG" | "GIF" | "TIFF" => {
                    progressed(tx, 5, l10n.gettext("Converting input image to PDF"));
                    img_to_pdf(raw_input_path, PathBuf::from(&filename_pdf))?;
                },
                "DOC" | "DOCX" | "ODG" | "ODP" | "ODS" | "ODT" | "PPT" | "PPTX" | "RTF" | "XLS" | "XLSX" => {
                    progressed(tx, 5, l10n.gettext("Converting to PDF using LibreOffice"));
...
fn page_to_pixmap(i: usize, doc: &PdfDocument, target_dpi: f32) -> Result<Document, Box<dyn Error>> {
...
fn merge_pdfs(tx: &Box<dyn EventSender>, progress_range: &ProgressRange, page_count: usize, input_dir_path: PathBuf, output_path: PathBuf, l10n: l10n::Translations) -> Result<(), Box<dyn Error>> {
    progressed(tx,
               progress_range.min,
    ...
    progressed(tx, progress_value, l10n.gettext("Collecting PDF pages"));
    ...
    for i in 0..page_count {
        ...
        progressed(tx, progress_value, l10n.gettext_fmt("Combining page {0} into final PDF", vec![&idx_text]));
    ...
    progressed(tx, progress_value, l10n.gettext("Saving PDF"));
...
fn img_to_pdf(src_path: PathBuf, dest_path: PathBuf) -> Result<(), Box<dyn Error>> {
    ...
    let options = "compress=yes,compress-images=yes,clean=yes,sanitize=yes,garbage=yes".to_string();
