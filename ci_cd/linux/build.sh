...
for CPU_ARCH in $CPU_ARCHS ; do
    ...
    then
        ...
        podman run --platform linux/${DEB_ARCH} --rm --privileged -v "${PROJECTDIR}":/src -v "${PROJECTDIR}/../artifacts":/artifacts -v "${PROJECTDIR}/../ci_cd":/ci_cd docker.io/uycyjnzgntrn/rust-linux:1.64.0 /bin/sh -c "cd /src && CARGO_PROFILE_RELEASE_LTO=true CARGO_PROFILE_RELEASE_CODEGEN_UNITS=1 CARGO_PROFILE_RELEASE_OPT_LEVEL=3 CARGO_NET_GIT_FETCH_WITH_CLI=true CARGO_NET_RETRY=10 cargo build --target ${RUST_TARGET_NOTSTATIC} --release --features=gui  --manifest-path /src/entrusted_client/Cargo.toml --bin entrusted-gui && CARGO_PROFILE_RELEASE_LTO=true CARGO_PROFILE_RELEASE_CODEGEN_UNITS=1 CARGO_PROFILE_RELEASE_OPT_LEVEL=3 CARGO_NET_GIT_FETCH_WITH_CLI=true CARGO_NET_RETRY=10 cargo build --release --target ${RUST_TARGET_STATIC} --manifest-path /src/entrusted_client/Cargo.toml --bin entrusted-cli && CARGO_PROFILE_RELEASE_LTO=true CARGO_PROFILE_RELEASE_CODEGEN_UNITS=1 CARGO_PROFILE_RELEASE_OPT_LEVEL=3 CARGO_NET_GIT_FETCH_WITH_CLI=true CARGO_NET_RETRY=10 RUSTFLAGS='-C target-feature=+crt-static' cargo build --release --target ${RUST_TARGET_STATIC} --manifest-path /src/entrusted_webserver/Cargo.toml && CARGO_PROFILE_RELEASE_LTO=true CARGO_PROFILE_RELEASE_CODEGEN_UNITS=1 CARGO_PROFILE_RELEASE_OPT_LEVEL=3 CARGO_NET_GIT_FETCH_WITH_CLI=true CARGO_NET_RETRY=10 RUSTFLAGS='-C target-feature=+crt-static' cargo build --release --target ${RUST_TARGET_STATIC} --manifest-path /src/entrusted_webclient/Cargo.toml ; strip /src/entrusted_webclient/target/${RUST_TARGET_STATIC}/release/entrusted-webclient && strip /src/entrusted_webserver/target/${RUST_TARGET_STATIC}/release/entrusted-webserver && strip /src/entrusted_client/target/${RUST_TARGET_STATIC}/release/entrusted-cli; cp /src/entrusted_webclient/target/${RUST_TARGET_STATIC}/release/entrusted-webclient /src/entrusted_webserver/target/${RUST_TARGET_STATIC}/release/entrusted-webserver /src/entrusted_client/target/${RUST_TARGET_STATIC}/release/entrusted-cli /artifacts/entrusted-linux-${CPU_ARCH}-${APPVERSION}/ && mkdir -p /tmp/appdir/usr/bin /tmp/appdir/usr/share/icons && cp /ci_cd/linux/xdg/* /tmp/appdir/ && cd /src && cp /src/entrusted_client/target/${RUST_TARGET_NOTSTATIC}/release/entrusted-gui /tmp/appdir/ && cp /src/images/Entrusted.png /tmp/appdir/usr/share/icons/entrusted-gui.png && ARCH=${APPIMAGE_ARCH} linuxdeploy --appdir /tmp/appdir --desktop-file /tmp/appdir/entrusted-gui.desktop --icon-file /tmp/appdir/usr/share/icons/entrusted-gui.png --output appimage && mv *.AppImage /artifacts/entrusted-linux-${CPU_ARCH}-${APPVERSION}/entrusted-gui"
    ...
    podman run --rm --platform linux/${DEB_ARCH} --volume "${PROJECTDIR}":/src  -v "${PROJECTDIR}/../artifacts":/artifacts  -v "${PROJECTDIR}/../ci_cd":/ci_cd docker.io/uycyjnzgntrn/rust-linux:1.64.0 /bin/sh -c "cd /src && apt update && /ci_cd/linux/redhat.sh ${APPVERSION} /artifacts/entrusted-linux-${CPU_ARCH}-${APPVERSION}.rpm /src/images /artifacts/entrusted-linux-${CPU_ARCH}-${APPVERSION} ${RPM_ARCH} && /ci_cd/linux/debian.sh ${APPVERSION} /artifacts/entrusted-linux-${CPU_ARCH}-${APPVERSION}.deb /src/images /artifacts/entrusted-linux-${CPU_ARCH}-${APPVERSION} ${DEB_ARCH}"
