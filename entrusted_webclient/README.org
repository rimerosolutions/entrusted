...
This repository provides a command-line client for the =entrusted-webserver= component.

There are few reasons to run the =entrusted-webclient= binary:
- You selected to install the =entrusted-webserver= on a remote machine
