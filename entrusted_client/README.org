...
In CLI mode (=entrusted-cli=), there's never any attempt to consider the =preview-result-appname= parameter. It's only intended for Desktop usage ("Save current settings as defaults" in the GUI client).
