...
The preference is to release with =seccomp= security profile features AND without random conversions failures...
- The root cause of conversion failures with =seccomp= is not yet known
- So far, it's fair to say that the application doesn't crash without =seccomp= custom security profiles...
