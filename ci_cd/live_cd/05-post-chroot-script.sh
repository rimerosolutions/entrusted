...
grub-mkstandalone --format=${EFI_ARCH}-efi  --directory=$HOME/LIVE_BOOT-${DEBIAN_ARCH}/chroot/usr/lib/grub/${EFI_ARCH}-efi --output=$HOME/LIVE_BOOT-${DEBIAN_ARCH}/tmp/boot${BOOT_EFI_ARCH}.efi --locales= --fonts= boot/grub/grub.cfg=$HOME/LIVE_BOOT-${DEBIAN_ARCH}/tmp/grub-standalone.cfg
