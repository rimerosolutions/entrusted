...
cp -r /usr/lib/grub/${EFI_ARCH}-efi/* $HOME/LIVE_BOOT-${DEBIAN_ARCH}/staging/boot/grub/${EFI_ARCH}-efi/

grub-mkstandalone --format=${EFI_ARCH}-efi --output=$HOME/LIVE_BOOT-${DEBIAN_ARCH}/tmp/boot-${EFI_ARCH}.efi --locales= --fonts= boot/grub/grub.cfg=$HOME/LIVE_BOOT-${DEBIAN_ARCH}/tmp/grub-standalone.cfg
(cd $HOME/LIVE_BOOT-${DEBIAN_ARCH}/staging/EFI/boot && dd if=/dev/zero of=efiboot.img bs=1M count=20 && /sbin/mkfs.vfat efiboot.img && mmd -i efiboot.img efi efi/boot && mcopy -vi efiboot.img $HOME/LIVE_BOOT-${DEBIAN_ARCH}/tmp/boot-${EFI_ARCH}.efi ::efi/boot/)
