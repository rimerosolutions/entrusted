...
podman run --privileged -v "${PROJECTDIR}":/src -v "${SCRIPTDIR}/appdir":/appdir -v "${PROJECTDIR}/artifacts":/artifacts docker.io/uycyjnzgntrn/rust-centos7:1.60.0 /bin/bash -c "ln -sf /usr/lib64/libfuse.so.2.9.2 /usr/lib/libfuse.so.2 && mkdir -p /appdir/usr/bin /appdir/usr/share/icons && cd /src/dangerzone_client && /root/.cargo/bin/cargo build --release --bin dangerzone-gui && cp target/release/dangerzone-gui /appdir/ && cp /appdir/dangerzone-gui.png /appdir/usr/share/icons/ && linuxdeploy --appdir /appdir --desktop-file /appdir/dangerzone-gui.desktop --icon-filename /appdir/dangerzone-gui.png --output appimage && mv *zone*.AppImage /artifacts/dangerzone-linux-amd64-${APPVERSION}/ && rm -rf /appdir/usr && rm /appdir/dangerzone-gui"
