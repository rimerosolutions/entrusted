...
for CPU_ARCH in $CPU_ARCHS ; do
    ...
    EXPORT_PARAMS="export CARGO_PROFILE_RELEASE_LTO=true; export CARGO_PROFILE_RELEASE_CODEGEN_UNITS=1; export CARGO_PROFILE_RELEASE_OPT_LEVEL=3; export CARGO_NET_GIT_FETCH_WITH_CLI=true; export CARGO_NET_RETRY=10; export CXX=/usr/local/osxcross/target/bin/o64-clang++; export CC=/usr/local/osxcross/target/bin/o64-clang;"
    STRIP_COMMAND="x86_64-apple-darwin21.4-strip"

    if [ ${CPU_ARCH} != "amd64" ]
    then
        RUST_TARGET="aarch64-apple-darwin"
        EXPORT_PARAMS="export CARGO_PROFILE_RELEASE_LTO=true; export CARGO_PROFILE_RELEASE_CODEGEN_UNITS=1; export CARGO_PROFILE_RELEASE_OPT_LEVEL=3; export CARGO_NET_GIT_FETCH_WITH_CLI=true; export CARGO_NET_RETRY=10; export CC=oa64-clang; export CXX=oa64-clang++;"    
