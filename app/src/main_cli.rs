...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    let (exec_handle, rx) = {
        ...
        let exec_handle = thread::spawn({
            move || {
                let convert_options = common::ConvertOptions::new(image_quality, ocr_lang, opt_passwd);
    ...
    if log_format == LOG_FORMAT_PLAIN {
        ...
        for line in rx {
            if let common::AppEvent::ConversionProgressEvent(increment, msg) = line {
