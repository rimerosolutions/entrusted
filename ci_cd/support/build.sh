...
podman rmi --force docker.io/uycyjnzgntrn/ubuntu:${DEBIAN_VERSION}-rust-${RUST_CI_VERSION}-amd64
podman rmi --force docker.io/uycyjnzgntrn/ubuntu:${DEBIAN_VERSION}-rust-${RUST_CI_VERSION}-arm64
podman rmi --force docker.io/uycyjnzgntrn/ubuntu:${DEBIAN_VERSION}-rust-${RUST_CI_VERSION}

buildah manifest create docker.io/uycyjnzgntrn/ubuntu:${DEBIAN_VERSION}-rust-${RUST_CI_VERSION}

buildah bud --squash --platform=linux/amd64 --format docker -t docker.io/uycyjnzgntrn/ubuntu:${DEBIAN_VERSION}-rust-${RUST_CI_VERSION}-amd64 -f Dockerfile.container.builder .
retVal=$?
if [ $retVal -ne 0 ]; then
	echo "Failed to build debian base container image for amd64"
  exit 1
fi
buildah manifest add docker.io/uycyjnzgntrn/ubuntu:${DEBIAN_VERSION}-rust-${RUST_CI_VERSION} docker.io/uycyjnzgntrn/ubuntu:${DEBIAN_VERSION}-rust-${RUST_CI_VERSION}-amd64

buildah bud --squash --platform=linux/arm64/v8 --format docker -t docker.io/uycyjnzgntrn/ubuntu:${DEBIAN_VERSION}-rust-${RUST_CI_VERSION}-arm64 -f Dockerfile.container.builder .
retVal=$?
if [ $retVal -ne 0 ]; then
	echo "Failed to build debian base container image for arm64"
  exit 1
fi
buildah manifest add docker.io/uycyjnzgntrn/ubuntu:${DEBIAN_VERSION}-rust-${RUST_CI_VERSION} docker.io/uycyjnzgntrn/ubuntu:${DEBIAN_VERSION}-rust-${RUST_CI_VERSION}-arm64
