...
for CPU_ARCH in ${CPU_ARCHS} ; do
    for LINUX_FLAVOR in ${LINUX_FLAVORS} ; do
        ARTIFACTSDIR="${PROJECTDIR}/../artifacts/entrusted-${APPVERSION}-linux-${LINUX_FLAVOR}-${CPU_ARCH}"
...
for CPU_ARCH in $CPU_ARCHS ; do    
    ...
    # MUSL
    mkdir -p "${PROJECTDIR}/../artifacts/entrusted-${APPVERSION}-linux-musl-${CPU_ARCH}"
    ...
    podman run --rm --platform linux/${DEB_ARCH} -v "${PROJECTDIR}":/src  -v "${PROJECTDIR}/../artifacts":/artifacts -v "${PROJECTDIR}/../ci_cd":/ci_cd docker.io/alpine:${ALPINE_VERSION} /bin/sh -c "apk add libxkbcommon-dev wayland-dev wayland-libs-client xwayland-dev rust cargo git cmake make g++ pango-dev fontconfig-dev libxinerama-dev libxfixes-dev libxcursor-dev && cargo clean --manifest-path /src/entrusted_client/Cargo.toml && cargo build --release --features=gui,fltk/use-wayland,fltk/fltk-shared --manifest-path /src/entrusted_client/Cargo.toml --bin entrusted-gui && cp /src/entrusted_client/target/release/entrusted-gui /artifacts/entrusted-${APPVERSION}-linux-musl-${CPU_ARCH}/ && strip /artifacts/entrusted-${APPVERSION}-linux-musl-${CPU_ARCH}/entrusted-gui"

    for LINUX_FLAVOR in ${LINUX_FLAVORS} ; do
        podman run --rm --platform linux/${DEB_ARCH} --volume "${PROJECTDIR}":/src  -v "${PROJECTDIR}/../artifacts":/artifacts  -v "${PROJECTDIR}/../ci_cd":/ci_cd docker.io/uycyjnzgntrn/rust-linux:${RUST_CI_VERSION} /bin/sh -c "cd /src && apt update && /ci_cd/linux/redhat.sh ${APPVERSION} /artifacts/entrusted-${APPVERSION}-linux-${LINUX_FLAVOR}-${CPU_ARCH}.rpm /src/images /artifacts/entrusted-${APPVERSION}-linux-${LINUX_FLAVOR}-${CPU_ARCH} ${RPM_ARCH} && /ci_cd/linux/debian.sh ${APPVERSION} /artifacts/entrusted-${APPVERSION}-linux-${LINUX_FLAVOR}-${CPU_ARCH}.deb /src/images /artifacts/entrusted-${APPVERSION}-linux-${LINUX_FLAVOR}-${CPU_ARCH} ${DEB_ARCH}" || (sleep 10 && podman run --rm --platform linux/${DEB_ARCH} --volume "${PROJECTDIR}":/src  -v "${PROJECTDIR}/../artifacts":/artifacts  -v "${PROJECTDIR}/../ci_cd":/ci_cd docker.io/uycyjnzgntrn/rust-linux:${RUST_CI_VERSION} /bin/sh -c "cd /src && apt update && /ci_cd/linux/redhat.sh ${APPVERSION} /artifacts/entrusted-${APPVERSION}-linux-${LINUX_FLAVOR}-${CPU_ARCH}.rpm /src/images /artifacts/entrusted-${APPVERSION}-linux-${LINUX_FLAVOR}-${CPU_ARCH} ${RPM_ARCH} && /ci_cd/linux/debian.sh ${APPVERSION} /artifacts/entrusted-${APPVERSION}-linux-${LINUX_FLAVOR}-${CPU_ARCH}.deb /src/images /artifacts/entrusted-${APPVERSION}-linux-${LINUX_FLAVOR}-${CPU_ARCH} ${DEB_ARCH}")
        ...
        cd ${ARTIFACTSDIR}/.. && tar cvf entrusted-${APPVERSION}-linux-${LINUX_FLAVOR}-${CPU_ARCH}.tar entrusted-${APPVERSION}-linux-${LINUX_FLAVOR}-${CPU_ARCH}    
    done
done
