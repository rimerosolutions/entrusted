...
pub fn convert(input_path: PathBuf, output_path: PathBuf, convert_options: common::ConvertOptions, tx: Box<dyn common::EventSender>,  trans: l10n::Translations) -> Result<bool, Box<dyn Error>> {
    ...
    if let Some(container_rt) = common::container_runtime_path() {
        ...
        // TODO there are occasional SIGSEGV with a custom seccomp profile apparently, this is occasional but relatively easy to reproduce with many files to convert (20 files)
        // Not sure exactly what is going on at this time, from workflow perspective, the final PDF is not found/produced properly or just dissapears???
        //
        //
        ...
        //    3.1 Write program with data structure to marshall seccomp JSON files (read/write)
        //    3.2 Create const array for all previously tested syscall names
        //    3.3 Create const array for all syscall names
        //    3.4 Write a loop that will disable selectively all the "non-previously tested syscalls" (ignore, marshall to json, run entrusted-cli, check for exit codes). When a test fails, we keep the syscall.
        // 4. Merge syscalls obtained from the oci hook (amd64) with the ones generated by the brute-force program on arm64
        // 5. Minify the seccomp JSON file
